...
FILE* FdopenPlatformFile(PlatformFile file, const char* modes) {
    if (file == kInvalidPlatformFileValue) return nullptr;
...
bool ClosePlatformFile(PlatformFile file) { return CloseHandle(file) != 0; }
...
bool ClosePlatformFile(PlatformFile file) { return close(file) == 0; }
