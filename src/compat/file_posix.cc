...
size_t File::Write(const uint8_t* data, size_t length) {
    ...
    do {
        ...
        do {
            written =
                ::write(file_, data + total_written, length - total_written);
...
size_t File::WriteAt(const uint8_t* data, size_t length, size_t offset) {
    ...
    do {
        ssize_t written;
        do {
            written = ::pwrite(file_, data + total_written,
                               length - total_written, offset + total_written);
        } while (written == -1 && errno == EINTR);
        if (written == -1) break;
        total_written += written;
    } while (total_written < length);
...
bool File::Close() {
    if (file_ == rtc::kInvalidPlatformFileValue) return false;
