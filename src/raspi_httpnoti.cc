...
void RaspiHttpNoti::OnResolveResult(rtc::AsyncResolverInterface* resolver) {
    if(resolver_->GetError() != 0) {
        ...
        RTC_LOG(INFO) << "Resolved server address : " << server_address_.ToString();
        DoIfAddrResolveConnect();
    }
}
...
void RaspiHttpNoti::DoIfAddrResolveConnect(void) {
    ...
    http_socket_->SignalCloseEvent.connect(this,
            &RaspiHttpNoti::OnIfAddrResolveClose);
...
void RaspiHttpNoti::DoConnect(void) {
    ...
    // connect socket signal
    http_socket_->SignalCloseEvent.connect(this,
            &RaspiHttpNoti::OnClose);
    http_socket_->SignalConnectEvent.connect(this,
            &RaspiHttpNoti::OnConnect);
    http_socket_->SignalReadEvent.connect(this,
            &RaspiHttpNoti::OnRead);
...
bool RaspiHttpNoti::GetHeaderValue(const std::string& data,
...
bool RaspiHttpNoti::ReadIntoBuffer(rtc::AsyncSocket* socket,
                                          std::string& data,
